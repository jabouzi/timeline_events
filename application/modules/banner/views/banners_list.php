<header>
	<ul>
		<?php foreach($languages as $code => $language) : ?>
			<li><?php echo anchor('banner/banners/'.$code, $language); ?></li>
		<? endforeach; ?>
	</ul>
</header>
		...
		<tbody id="workflow_list">
			<?php foreach ($banners->result() as $item) : ?>
				<tr>
					...
					<?php if ($this->session->userdata('user_permission') <= 2) : ?>
					<td>						
					<?php echo anchor('banner/editbanner/'.$item->campaign_banner_id.'/'.$language_code, '<img  src="'.site_url().'assets/images/icn_edit.png" title="'.lang('banner.edit').'">'); ?>						
